...
library(sf)
#attw
spp_list <-  c("piwo", "nofl", "dowo",  "hawo", "rbsa", "acwo", "attw", "whwo", "wisa", "rnsa", "bbwo")
ecoregion <- "nwmf"
for(sp in spp_list){
  pred_rst <- rast(paste0("D:/hotspot/output/ecoregion/rf-model_encounter-rate_", sp, "_", ecoregion, "_clip.tif"))
  ...
  reclass_m <- matrix(reclass_df,
                      ncol = 3,
                      byrow = FALSE)
  quant_nm <- paste0("D:/hotspot/output/ecoregion/quants_", sp, "_", ecoregion, "_clip.tif")
  quants_rst <- classify(pred_rst,
                          rcl = reclass_m, 
                          filename = quant_nm,
                          overwrite = TRUE)
  plot(quants_rst, bty="n", axes = FALSE, box=FALSE)
  plot_nm <- paste0("D:/hotspot/output/ecoregion/quantile_", sp, "_", ecoregion, "_clip.png")
  png(plot_nm, height=nrow(quants_rst), width=ncol(quants_rst)) 
  plot(quants_rst, bty="n", axes = FALSE, box=FALSE, legend = FALSE)
  dev.off()
  test_name <- paste0("D:/hotspot/", sp, "_", ecoregion, "_test_clipped.csv")
  ...
  extr <- terra::extract(quants_rst, vect(ebird_proj), progress = TRUE)
  ...
  reclass_m2 <- matrix(reclass_df2,
                      ncol = 2,
                      byrow = FALSE)
  binary_nm <- paste0("D:/hotspot/output/ecoregion/binary_", sp, "_", ecoregion, "_clip.tif")
