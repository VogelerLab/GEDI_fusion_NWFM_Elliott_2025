...
library(scam)
library(ranger)
spp <- c("piwo", "nofl", "dowo",  "hawo", "rbsa", "acwo", "attw", "whwo", "wisa", "rnsa", "bbwo")
ecoregion <- c("nwmf")
...
for(sp in spp){
  ...
  ecoregions <- c("nwmf")#,"mwcf", "nade", "grpl", "nwmf")
  for(er in 1:length(ecoregions)){
    ...
    writeRaster(pred_rf_true, paste0("D:/hotspot/output/ecoregion/pred_", sp, "_", ecoregions[er], "_clip.tif"), overwrite = TRUE)
    pred_rf_cal <- terra::predict(pred_rf_true, calibration_model, 
                                  type = "response", cpkgs = "scam", na.rm = TRUE)
    pred_name <- paste0("D:/hotspot/output/ecoregion/rf-model_encounter-rate_", sp, "_", ecoregions[er], "_clip.tif")
