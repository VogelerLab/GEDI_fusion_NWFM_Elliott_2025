...
# ebird data
script_dir <- ("C:/Users/lisah/OneDrive/Documents/GEDI Work/data/hotspot/")
data_name_ebird <- paste0(script_dir, "ebird/all_ebird_ecoregion.csv")
ebird <- read_csv(data_name_ebird)

# modis covariates
modis_dat <- read_csv(paste0(script_dir, "data/modis_pland_location-year_hotspot4.csv")) %>% 
  mutate(year = as.integer(year))

#gedi covariates
gedi_dat <- read_csv(paste0(script_dir,"data/gedi_location-year_hotspot.csv"))
...
#bioclim covariates
bioclim_dat <- read_csv(paste0(script_dir,"data/bioclim_data_800m.csv"))
...
# prediction surface
#pred_surface <- read_csv(paste0(script_dir, "data/pland-elev_prediction-surface.csv"))
# latest year of landcover data
max_lc_year <- 2020
r <- raster(paste0(script_dir, "data/fhd_90m.tif"))
...
sp_list <- c("piwo", "nofl", "dowo", "whwo", "hawo", "wisa", 
...
for(sp in 1:length(sp_list)){
  ...
  for(er in 1:length(ecoregion_list)){
    ...
    nm <- paste0("D:/hotspot/obs_", sp_list[sp], "_", ecoregion_list[er], ".csv")
    write.csv(obs, nm)
  }
}
...
for(sp in 1:length(sp_list)){
  ...
  writeVector(data_clipped, filename = paste0("D:/hotspot/obs_", sp_list[sp], "_", ecoregion, "_clipped.shp"))
...
for(sp in 1:length(sp_list)){
  ...
  for(er in 1:length(ecoregion_list)){
    print(ecoregion_list[er])
    obs <- read.csv(paste0("D:/hotspot/obs_", sp_list[sp], "_", ecoregion, "_clipped.csv"))
    obs_spat <- vect(paste0("D:/hotspot/obs_", sp_list[sp], "_", ecoregion, "_clipped.shp"))
    ...
    if(sum(obs$species_observed) >= 30){
      ...
      coords <- as.data.frame(geom(obs_spat))
      
      # Combine coordinates with original attributes
      # Note: The `geom` function extracts coordinates as columns 'x' and 'y' by default
      obs_df <- cbind(obs, coords)
      
      # Rename coordinates columns to 'longitude' and 'latitude'
      obs_df <- obs_df %>%
        rename(longitude = x, latitude = y)
      
    
      # get hexagonal cell id and week number for each checklist
      checklist_cell <- obs_df %>% 
      ...
      #67550
      #> [1] 121017
      b_samp <- count(obs_df, species_observed) %>% 
      ...
      #########
      setwd("D:/hotspot")
      train_name <- paste0(sp_list[sp], "_", ecoregion_list[er], "_train_clipped.csv")
      test_name <- paste0(sp_list[sp], "_", ecoregion_list[er], "_test_clipped.csv")
...
write.csv(sp_stats, "D:/hotspot/sp_stats_ecoregion_nwmf_clip.csv", row.names = FALSE)
