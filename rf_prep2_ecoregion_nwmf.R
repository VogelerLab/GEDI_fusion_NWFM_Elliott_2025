...
#set sp
spp <- "rnsa"
#spp <- c("piwo", "nofl", "dowo", "whwo", "bbwo", "wisa", "attw", "acwo", "hawo", "rbsa", "rnsa")
ecoregion_list <- c("nwmf")#, "mwcf", "grpl", "nade", "nwmf")
for(i in 10:length(spp)){
  ...
  #set names
    train_name <- paste0(spp[i], "_", ecoregion_list[er], "_train_clipped.csv")
    test_name <- paste0(spp[i], "_", ecoregion_list[er], "_test_clipped.csv")
    ...
    if(file.exists(file.path(dDir, train_name)) == TRUE){
      ...
      cor_name <- paste0(sp, "_CorMatrix_rf_clipped.csv")
      write.csv(cormat, file=paste0("D:/hotspot/", cor_name))
      ...
      aliased_vars <- rownames(alias_result$Complete)
      print(aliased_vars)
      bv_list <- biovar_list
      for(bv in 1:length(bv_list)){
        # Remove aliased variables if necessary
        if (biovar_list[bv] %in% aliased_vars) {
          # Fit the model without the aliased variable
          bv_list <- biovar_list[-bv]
          fun <- paste("species_observed ~ ", paste(bv_list, collapse = " + "))
          model <- lm(fun,
                      data = spDat)
          print("Aliased variable removed and updated model fitted.")
        } else {
          model <- model
          bv_list <- bv_list
        }
      ...
      if(any(vif(model) >= 10)){
        ...
        biovar_list <- bv_list[which(!bv_list %in% names(which.min(rf$variable.importance)))]
        fun <- paste("species_observed ~ ", paste(bv_list, collapse = " + "))
        ...
        if(any(vif(model) >= 10)){
          ...
          bv_list <- bv_list[which(!bv_list %in% names(which.min(imp)))]
          fun <- paste("species_observed ~ ", paste(bv_list, collapse = " + "))
          model <- lm(fun, data = spDat)
          summary(model)
          vif(model)
          ...
          if(any(vif(model) >= 10)){
            ...
            bv_list <- bv_list[which(!bv_list %in% names(which.min(imp2)))]
            fun <- paste("species_observed ~ ", paste(bv_list, collapse = " + "))
            model <- lm(fun, data = spDat)
            summary(model)
            vif(model)
            ...
            if(any(vif(model) >= 10)){
              ...
              bv_list <- bv_list[which(!bv_list %in% names(which.min(imp3)))]
              fun <- paste("species_observed ~ ", paste(bv_list, collapse = " + "))
              model <- lm(fun, data = spDat)
              summary(model)
              vif(model)
              ...
              if(any(vif(model) >= 10)){
                ...
                bv_list <- bv_list[which(!bv_list %in% names(which.min(imp4)))]
                fun <- paste("species_observed ~ ", paste(bv_list, collapse = " + "))
                model <- lm(fun, data = spDat)
                summary(model)
                vif(model)
                ...
                if(any(vif(model) >= 10)){
                  ...
                  bv_list <- bv_list[which(!bv_list %in% names(which.min(imp5)))]
                  fun <- paste("species_observed ~ ", paste(bv_list, collapse = " + "))
                  model <- lm(fun, data = spDat)
                  summary(model)
                  vif(model)
    ...
    vars_list <- c(vars_for_all, e_list, d_list, bv_list, c_list, f_list, r_list)
    vars_name <- paste0(dDir, "/", sp, "_", ecoregion_list[er], "_vars_list3_clip.csv")
