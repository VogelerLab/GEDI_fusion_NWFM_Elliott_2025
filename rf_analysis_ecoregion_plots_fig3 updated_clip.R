...
#set plot_grp
#plot_grp <- 1 #plot just the species that occur in the nwfm
plot_grp <- 2 #plot species that occur in the nwfm and mwcf
#plot_grp <- 3 #plot species that occur in the nwfm, mwcf, and 6 state ecoregion
#plot_grp <- 4 #plot species of interest for the poster (ACWO, NOFL) that occur in the nwfm and mwcf
#plot_grp <- 5 #plot the species that occur in the 6 state ecoregion but ONLY for nwfm and mwcf

#set ecoregion and species list
if(plot_grp == 1){
  er <- c("nwmf")
  erCAP <- c("NWFM")
  spp <-c("whwo",  "wisa", "rnsa", "bbwo") #c("piwo", "nofl", "dowo", "hawo",  "rbsa", "acwo", "attw")
  sppCAP <- c("WHWO",  "WISA", "RNSA", "BBWO")
  pd_name <- "pd_panelA.jpg"
  ht <- 5
} else if(plot_grp == 2){
  er <- c("nwmf", "mwcf")
  erCAP <- c("NWFM", "MWCF")
  spp <-c("hawo",  "rbsa", "acwo", "attw") 
  sppCAP <- c("HAWO",  "RBSA", "ACWO", "ATTW")
  pd_name <- "pd_panelB.jpg"
  ht <- 10
  } else if(plot_grp == 3){
    er <- c("nwmf", "mwcf", "all")
    erCAP <- c("NWFM", "MWCF", "all")
    spp <-c("piwo", "nofl", "dowo")
    sppCAP <- c("PIWO",  "NOFL", "DOWO")
    pd_name <- "pd_panelC.jpg"
    ht <- 7
    } else if(plot_grp == 4){
      er <- c("nwmf", "mwcf")
      erCAP <- c("NWFM", "MWCF")
      spp <-c("acwo", "nofl") #c("piwo", "nofl", "dowo", "hawo",  "rbsa", "acwo", "attw")
      sppCAP <- c("ACWO", "NOFL")
      pd_name <- "pd_poster.jpg"
      ht <- 5
    } else if(plot_grp == 5){
      er <- c("nwmf", "mwcf")
      erCAP <- c("NWFM", "MWCF")
      spp <-c("piwo", "nofl", "dowo") #c("piwo", "nofl", "dowo", "hawo",  "rbsa", "acwo", "attw")
      sppCAP <- c("PIWO", "NOFL", "DOWO")
      pd_name <- "pd_panelC_placeholder.jpg"
      ht <- 7
    }
#set ecoregion
...
vars_list <- c("rh98_90", "rh98_180",  "rh98_360", "rh98_540", "rh98_1530", "rh98_2520",
               ...
               "biovars_1", "biovars_2", "biovars_3", "biovars_4", "biovars_5",
               "biovars_6", "biovars_7", "biovars_8", "biovars_9", "biovars_10",
               "biovars_11", "biovars_12", "biovars_13", "biovars_14", "biovars_15", 
               "biovars_16","biovars_17", "biovars_18", "biovars_19",
...
for(i in 1:length(spp)){
  ...
  for(j in 1:length(er)){
    ...
    } else if(ecoregion == "nwmf"){
      train_name <- paste0(sp, "_", ecoregion, "_train_clipped.csv")
      test_name <- paste0(sp, "_", ecoregion, "_test_clipped.csv")
      vars_name <- paste0(dDir, "/", sp, "_", ecoregion, "_vars_list3_clip.csv")
      rfname <- paste0("RF_models/", sp, "_", ecoregion, "_clip.rds")
    }
    ...
    outputDir <- file.path(baseDir, "hotspot analysis/nwfm")
    ...
    average_encounter <- rf_pred_train %>%
      ...
      group_by(pred_cat) %>%
      mutate(dummy = 1) %>%
      summarise(pred = mean(pred), obs = mean(obs), checklist_count = sum(dummy)) %>%
      ungroup()
    ...
    pd_sp <- pd %>%
      ...
      mutate("Ecoregion" = ecoregionCAP) %>%
      mutate("Key" = paste(Species, Ecoregion)) %>%
...
pd_all_gedi$predictor <- factor(pd_all_gedi$predictor, levels= c(unique(pd_all_gedi$predictor)))
...
pd_plot <- ggplot(pd_all_gedi) +
  aes(x = value, y = encounter_rate, color = Buffer, linetype = Buffer) +
  geom_line () +
  geom_point() +
  scale_y_continuous(labels = scales::percent) +
  facet_grid(Key ~ as_factor(Bin), scales = "free") +
  ...
  theme(panel.grid = element_blank(),
        axis.line = element_line(color = "grey60"),
        axis.ticks  = element_line(color = "grey60"))+
  theme(axis.text=element_text(size=10))#NEW parameter
ggsave(filename = pd_name, 
       path = "C:/Users/lisah/Documents/GEDI Work/hotspot analysis/nwmf", 
       width = 6, height = ht, dpi = 600)
...
pd_all_bioclim <- pd_all_grp %>%
  filter(Category == "Bioclimatic") %>%
  filter(predictor == c("biovars_4", "biovars_5", "biovars_8", "biovars_15")) %>%
  mutate(Buffer = c("800m"))

pd_all_bioclim$predictor <- factor(pd_all_bioclim$predictor, 
                                   levels= c("biovars_4", "biovars_5",
                                             "biovars_8", "biovars_15"), 
                                   ordered = TRUE)
pd_all_bioclim$Species <- factor(pd_all_bioclim$Species, levels= sppCAP, 
                                 ordered = TRUE)

###
#pdp_name <- paste0(outputDir, "/Partial_Dep_Plot_", sp, "_", ecoregion, ".png")

bioclim_pd_name <- paste0("bioclim_", pd_name)

library(paletteer)
paletteer_c("ggthemes::Blue-Green Sequential", 7)
paletteer_c("grDevices::YlGnBu", 14)
pd_plot <- ggplot(pd_all_bioclim) +
  aes(x = value, y = encounter_rate, color = Buffer, linetype = Buffer) +
  geom_line () +
  geom_point() +
  scale_y_continuous(labels = scales::percent) +
  facet_grid(Key ~ as_factor(predictor), scales = "free") +
  labs(y = "Encounter Rate") +
  theme_minimal() +
  theme_minimal() +
  scale_colour_manual(values=c("#0076AFFF")) +
  theme(plot.title.position = "plot") +
  theme(panel.grid = element_blank(),
        axis.line = element_line(color = "grey60"),
        axis.ticks  = element_line(color = "grey60"))+
  theme(axis.text=element_text(size=10))#NEW parameter
ggsave(filename = bioclim_pd_name, 
       path = "C:/Users/lisah/Documents/GEDI Work/hotspot analysis/nwmf", 
       width = 6, height = ht, dpi = 600)
