...
#source(file.path(script_dir, "clip to range.R")) #to clip the dataset to only the range of the species of interest
source(file.path(script_dir, "dataset_for_RF2_ecoregion_nwmf_clip.R")) #to combine all data, clip the dataset to only the range of the species of interest, and set up test/training data for all 



source(file.path(script_dir, "rf_prep2_ecoregion_nwmf.R")) #select vars for rf...this will need to be hand selected for each species
source(file.path(script_dir, "rf_analysis_ecoregion_clip.R")) #complete rf for the final analysis
...
source(file.path(script_dir, "rf_prediction_ecoregion_nwmf_clip.R")) #complete spatial predictions
source(file.path(script_dir, "ecoregion_quantiles_nwmf_clip.R")) #to find thresholds to get binary grids
source(file.path(script_dir, "hotspots_ecoregion_nwmf_clip.R")) #combine individual species maps to get species richness and hotspots
